...
export const Route = createFileRoute("/examples/placements")({
...
function PlacementToast() {
  ...
  const { ref, className, handlers, attributes } = useToastAnimation({
    className:
      "pointer-events-auto relative w-full rounded-3xl border border-[var(--line)] bg-[var(--surface-strong)] p-4 pr-12 text-[var(--ink)] shadow-[0_18px_36px_rgba(15,23,42,0.12)]",
  });

  return (
    <article ref={ref} className={className} {...handlers} {...attributes}>
      <p className="text-xs font-bold tracking-[0.18em] uppercase text-[var(--accent-strong)]">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {placements.map((placement) => (
          <button
            key={placement}
            type="button"
            className="rounded-2xl border border-[var(--line)] bg-[var(--chip-bg)] px-4 py-3 text-left text-sm font-semibold text-[var(--ink)] hover:-translate-y-0.5"
            onClick={() =>
              toast.info(
                {
                  title: placement,
