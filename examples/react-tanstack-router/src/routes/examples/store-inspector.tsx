...
export const Route = createFileRoute("/examples/store-inspector")({
...
function StateTable({
  ...
  return (
    <div className="rounded-3xl border border-[var(--line)] bg-[var(--chip-bg)] p-4">
      <h3 className="text-sm font-semibold text-[var(--ink)]">
        {label} — {toasts.length} toast(s)
      </h3>
      {toasts.length === 0 ? (
        <p className="mt-2 text-sm text-[var(--ink-soft)]">No active toasts.</p>
      ) : (
        <div className="mt-3 overflow-x-auto">
          <table className="min-w-full text-left text-xs text-[var(--ink-soft)]">
            <thead>
            ...
            <tbody>
              {toasts.map((toast) => (
                <tr key={toast.id} className="border-b border-[var(--line)]/70">
