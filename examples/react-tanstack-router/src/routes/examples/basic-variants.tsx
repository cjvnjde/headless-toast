...
export const Route = createFileRoute("/examples/basic-variants")({
...
function VariantToast() {
  ...
  const { ref, className, handlers, attributes } = useToastAnimation({
    className:
      "pointer-events-auto relative w-full rounded-3xl border border-white/60 bg-white/95 p-4 pr-12 shadow-[0_20px_40px_rgba(15,23,42,0.12)] backdrop-blur dark:border-white/10 dark:bg-slate-950/90",
  });
...
function BasicVariantsPreview() {
  ...
  const buttonClass =
    "rounded-full border border-[var(--line)] bg-[var(--chip-bg)] px-4 py-2 text-sm font-semibold text-[var(--ink)] hover:-translate-y-0.5";
